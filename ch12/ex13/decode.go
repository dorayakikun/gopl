...
func fieldName(tag string, v reflect.Value) string {
	for i := 0; i < v.NumField(); i++ {
		t := v.Type().Field(i).Tag.Get("sexpr")
		if t == tag {
			return v.Type().Field(i).Name
		}
	}
	panic(fmt.Sprintf("field not found by tag: %s", tag))
}
...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name value) ...)
		for !endList(lex) {
			...
			name := lex.text()
			lex.next()
			read(lex, v.FieldByName(fieldName(name, v)))
			lex.consume(')')
		}
